...
clearonecache(lua_State *L) {
    ...
    if (strlen <= 0) {
      luaL_error(L, "clearone not find str!!!");
    }
    lua_pushstring(CC.L, str);
    lua_pushnil(CC.L);
    lua_rawset(CC.L, LUA_REGISTRYINDEX);
  SPIN_UNLOCK(&CC)
}

static void
printcache(lua_State *L) {
	if (CC.L == NULL || L == NULL)
		return;
