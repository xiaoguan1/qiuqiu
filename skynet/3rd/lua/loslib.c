...
/* }====================================================== 
  2024.1.29 ggw create stat
*/
#include <sys/stat.h>

static int stat_filetype (lua_State *L) {
  size_t sz = 0;
  const char* filepath= luaL_checklstring(L, 1, &sz);
  if (sz <= 0) {
      return luaL_error(L, "not args!");
  }
  struct stat buf;
  int result;
...
static const luaL_Reg stat_syslib[] = {
  {"filetype",    stat_filetype},
  {NULL, NULL}
};

LUAMOD_API int luaopen_stat (lua_State *L) {
  luaL_newlib(L, stat_syslib);
  return 1;
}
